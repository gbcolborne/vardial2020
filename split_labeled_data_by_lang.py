...
def main():
    ...
    # Check args
    assert "train.tsv" in os.listdir(input_dir) and "valid.tsv" in os.listdir(input_dir)
    if os.path.exists(args.output_dir):
        assert os.path.isdir(args.output_dir) and len(os.listdir(args.output_dir)) == 0
    else:
        os.makedirs(args.output_dir)
    parts = ["train", "valid"]
    if "test.tsv" in os.listdir(input_dir):
        parts.append("test")

    # Process data
    for part in parts:
        lang2out = {lang:open("%s.%s" % (lang, part), 'w') for lang in ALL_LANGS}
        fni = "%s.tsv" % part
        with open(fni) as fi:
            for line in fi:

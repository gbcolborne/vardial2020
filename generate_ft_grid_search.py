...
def main():
    ...
    settings["sampling_distro"] = ["uniform", "relfreq", "dampfreq"]
    
    # Enumerate combinations
    combinations = [[]]
    for k in ["train_batch_size", "seq_len", "learning_rate", "grad_accum_steps", "sampling_distro"]:
        new_combinations = []
    ...
    # Check combinations
    filtered = []
    for (bs, sl, lr, gs, sd) in combinations:
        if bs == "32" and sl == "256":
            continue
        filtered.append((bs, sl, lr, gs, sd))
    ...
    for i, (bs, sl, lr, gs, sd) in enumerate(combinations):
        # Encode settings in output directory path
        dir_output = "FT_exp2_bs=%s_sl=%s_lr=%s_gs=%s_sd=%s" % (bs, sl, lr, gs, sd)        

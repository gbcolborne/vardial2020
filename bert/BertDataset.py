...
class BertDatasetForTraining(IterableDataset):
    ...
    def __init__(self, train_paths, tokenizer, seq_len, sampling_alpha=1.0, weight_relevant=1.0, encoding="utf-8", seed=None, verbose=False):
        ...
        logger.info("Skipping random number of lines in each corpus")
        for lang in self.lang_list:
    ...
    def prep_files_for_streaming(self):
        """ Make sure files are open and skip to current line """
        logger.info("Preparing files for streaming")
        for lang in self.lang_list:
            path = self.lang2path[lang]
            self.lang2file[lang] = open(path, 'r', encoding=self.encoding)
            start = self.lang2ix[lang]
            if start > 0:
                logger.info("  %s: skipping to line %d" % (lang, start))
